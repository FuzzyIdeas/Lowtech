...
public enum HashidsOptions {
    static let VERSION = "1.1.0"
    static var MIN_ALPHABET_LENGTH = 16
    static var SEP_DIV = 3.5
    static var GUARD_DIV: Double = 12
    static var ALPHABET = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"
    static var SEPARATORS = "cfhistuCFHISTU"
}

// MARK: - HashidsGenerator
...
open class Hashids_<T>: HashidsGenerator where T: UnsignedInteger {
    public init(salt: String!, minHashLength: UInt = 0, alphabet: String? = nil) {
        ...
        seps = intersection(seps, self.alphabet)
        ...
        if (sepsLength == 0) || (Double(alphabetLength) / Double(sepsLength) > HashidsOptions.SEP_DIV) {
    ...
    fileprivate func _decode(_ hash: [Char]) -> [Int] {
        ...
        if hash.count > 0 {
            ...
            for subHash in valueHashes {
                shuffle(&alphabet, lsalt, lsaltRange)
                ret.append(_unhash(subHash, alphabet))
                lsalt.replaceSubrange(lsaltARange, with: alphabet)
            }
        }
